...
$q_\mathrm{b}$ & $60~\mathrm{mA}/508.6~\mathrm{MHz} = 1.2 \times 10^{-13}~\mathrm{C}$ \\
...
\begin{equation}
E_x = \frac{q_\mathrm{b} \cdot x}{2\pi \epsilon_0 l_\mathrm{b}\sigma_y(\sigma_x + \sigma_y)}.
\end{equation}

ただし，ここで$l_\mathrm{b}$ はバンチのz 方向の大きさのスケールである．
これに各値を代入すると，$E_x$ として$\sim 62400~[\mathrm{V}] /l_\mathrm{b}~[\mathrm{mm}]$ が得られる．
$l_\mathrm{b}$ は$\sigma_z$ の大きさ程度であると考えられるので，結果として数~MV/m という値が得られ，これがワイヤの位置が感じる$x$ 方向の電場の最大値であると考えられる．
